...
**Federated session ids.** A `sessionId` of the form `name:localId` (see `formatForeignSessionId` in `core/foreign-session-id.ts`) names a session owned by the federated peer registered under `name` (see [Remotes](#remotes)) rather than this daemon. Any route below shaped `/v1/sessions/:id...` transparently forwards to that peer using this daemon's own stored peer credential when given such an id — the caller never needs its own credential for the peer, and never sees the peer's raw host/port. Unrecognized/local-looking ids (no colon) are handled locally first, as always. Only on a confirmed local miss (this daemon has no session under that raw id) does `session/attach` and every `/v1/sessions/:id...` route fall back to checking whether exactly one federated peer has a session under that same raw id — via the already-warm `ForeignSessionCache`, so this costs nothing beyond the miss check itself for the normal, locally-resolvable case. Zero matches falls through to the ordinary "not found" error; more than one (practically unreachable given the id alphabet, but a defined outcome) answers with an error naming every matching remote instead of guessing. A match forwards exactly as if the caller had typed `<name>:<id>` in the first place. An entry merged in from a peer (see `GET /v1/sessions` below) also carries `"remote": "<name>"`. This is deliberately a different field from `importedFromMachine` (set on a cold bundle-imported record — see `POST /v1/sessions/import`): `remote` marks a *live* session that stays live on the peer, `importedFromMachine` marks a static copy sitting locally; a client that folds both into one "which host" picker/dropdown still needs to branch on which field is set before deciding what clicking the entry should do.
...
`GET /v1/sessions` additionally merges in every federated peer's own list (each entry's `sessionId` rewritten to the `name:localId` form above) whenever the call has no `cwd` filter — `cwd` is inherently local-machine-scoped and skips the merge, a peer's tree means nothing against it. This applies equally to an incremental (`since=`) poll: skipping the merge there was the original, shipped behavior, on the reasoning that `since=` is local-cursor-scoped, but in practice `since=` is what every client actually uses after its first request, so skipping the merge there meant federated sessions appeared exactly once, on a client's very first load, and never again.

The merge reads from `daemon/routes/session-forward.ts`'s `ForeignSessionCache` rather than fetching each peer live per caller request. That cache maintains its **own** cursor per peer, independent of any caller's cursor (a caller's `since` is minted by, and only meaningful to, whichever daemon issued it — it can never be forwarded to a peer directly), and refreshes by calling *that peer's* `GET /v1/sessions?since=<our-remembered-cursor>` on a background timer (every 5s by default) — the same incremental mechanism a normal client already uses against us, just one level out. This means a peer's full session list is only ever fetched once (when first federated, or after this daemon restarts); every refresh after that is a real incremental delta, merged into the cache with the identical rule the client-side merge already uses (see the `since=` bullet above): purge anything not definitively cold, delete `removed` ids, upsert what the page sent. Reads from the cache are cheap enough to serve on every single `GET /v1/sessions` call regardless of the caller's own `since`, and centralizing the peer fetch here means N local clients polling costs one fetch per peer per refresh interval, not N.

The cache does **not** filter out a session that's on the peer only because *the peer* imported a bundle for it and never launched it there (`importedFromMachine` set, `upstreamSessionId` not — see `POST /v1/sessions/import`). That's dormant, unclaimed data sitting on the peer, not "something happening on it," but `GET /v1/sessions` is a general-purpose data surface — a local dormant mirror isn't excluded from it either, so a federated one isn't singled out for exclusion here. Whether it's worth *showing by default* is a presentation decision, made client-side: the TUI picker and browser both drop a remote's dormant mirrors from that remote's own bucket while still surfacing them under "all" (mirroring how a local dormant mirror is already dropped from "local" but not "all"). Once the peer itself attaches to one (`upstreamSessionId` gets set there), it's indistinguishable from any other live session on the next refresh.

An unreachable peer just leaves its cache entry as of the last successful refresh (stale, not wiped) rather than failing the whole listing or forcing a full re-fetch once it recovers — a stored cursor stays valid indefinitely (see the `since=` bullet above), so resuming incrementally after any length of downtime is safe. `GET /v1/remotes`'s `status` (see [Remotes](#remotes)) is the place to look for *why* a peer's sessions might be stale.
...
- `since=<cursor>` — incremental listing. Pass the `cursor` from a previous response and the daemon returns **every warm entry** plus **only the cold entries whose record changed at or after that cursor**, with `removed` naming the sessions deleted since. The response shape is unchanged; only the contents narrow. A client polling the list should use this: a full listing reads and validates every record on disk (tens of MB on a long-lived install), an incremental one reads only what changed. Combines with `cwd`, `includeNonInteractive` and `status` as usual. `400` if the value is not a non-negative number.
  ...
  Federated entries (see below) are the one exception to "only what changed": every response, incremental or not, carries every currently-cached entry from every peer, not a delta — there's no cross-daemon cursor to narrow by. A client's merge logic should upsert them the same as any other row in the response; nothing marks them as "unchanged, skip."
...
`session/attach`, `session/detach`, `session/prompt`, `session/cancel`, `session/close`, `session/delete` (and its `hydra-acp/session/delete` alias), `session/set_model`, `session/set_mode`, `session/set_config_option`, `_session/steering`, `hydra-acp/prompt/{cancel,update,amend}`, `hydra-acp/session/tool_content`, and `hydra-acp/session/force_cancel` addressed at a federated session id (`name:localId` — see [Remotes](#remotes) and [Sessions](#sessions)) are transparently forwarded to the peer registered under `name`, using this daemon's own stored peer credential (`daemon/acp-forward.ts`). To the peer, this looks like an ordinary client attaching over its normal `/acp` endpoint — no protocol extension is required or visible on the peer's side. `session/attach` additionally accepts a *bare* id (no `name:` prefix) as a local-miss fallback: if this daemon has no session under that raw id, it checks whether exactly one known peer does (via the already-warm federated session cache, no extra network round trip) and forwards there as if `name:` had been typed; zero matches falls through to the ordinary not-found error, more than one names every matching remote instead of guessing.

The peer's own outbound traffic on that dedicated connection relays back the other direction generically: every notification the peer sends carrying a top-level `sessionId` that matches (`session/update` and the whole `hydra-acp/{prompt_queue/*,cancel_failed,session/armed_tasks_updated,session/closed}` family) is forwarded to the local target with the id rewrapped to `name:localId`, and `hydra-acp/session/closed` additionally clears the registration. This is a catch-all (`JsonRpcConnection.onAnyNotification`), not a per-method list, so nothing new added to `Session`'s broadcast surface needs a matching update in `acp-forward.ts`. `hydra-acp/session/request_permission` (a peer-initiated *request*, not a notification) is relayed the same way, one method at a time, since it needs a reply routed back upstream rather than a bare forward.

This relay is wired up before `session/attach` is sent to the peer, not after its response resolves. The peer can push notifications on this connection while the attach call is still in flight (a readonly/cold attach's history replay, for example), and a live-only catch-all has nothing to drain if it starts listening after those already arrived: the attached client would come up with an empty transcript even though the peer sent everything correctly.

Deliberately **not** forwardable: `session/fork`, `session/resume`, and `session/load` bind a *new* local attachment and hand back a sessionId that becomes the addressing target for everything after it — forwarding them generically (the way the methods above do) would return an unaddressable bare id with no corresponding registry entry, silently breaking every follow-up call. `hydra-acp/attention/{set,clear}` are registered only for transformer-kind connections (see `TRANSFORMER_ONLY_HYDRA_ACP_METHODS` in `acp-ws.ts`); forwarding runs *before* that local gate, so including them would let any caller reach a transformer-only method on a federated session through this daemon's own peer credential.
