...
`tui.defaultHost` (default `"remote:all"`) sets which host bucket the session picker opens on, before `h` cycles it. `"remote:all"` is local plus every `hydra remote`'s live sessions without imported-machine mirrors, so with no live remote it looks the same as `"local"`. `"local"` is sessions created or bound on this machine, and `"all"` drops the host filter. `"remote:<name>"` and `"host:<name>"` pick one remote or one imported machine; a bare name is a remote of that name, else an imported machine. A value that nothing backs when the picker opens shows local and is retried on each open until it resolves or you press `h`. TUI-only: `hydra session list --host` keeps its own `"local"` default.
