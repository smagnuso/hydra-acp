  ...
  switch (subcommand) {
    ...
    case "cat": {
      ...
      const catOpts: Parameters<typeof runCat>[0] = {
        ...
        noFromNote: flags["no-from-note"] === true,
        dangerouslySkipPermissions,
      };
      if (cwd !== undefined) {
        catOpts.cwd = cwd;
...
function printHelp(subcommand?: string): void {
  const lines: readonly HelpLine[] = [
    ...
    [CAT, "  hydra-acp cat [-p <prompt>] [--session <id-or-url>] [--detach] [--raw] [--no-wait] [--timeout <s>] [--no-from-note] [--agent <id>] [--model <id>] [--name <label>]"],
    ...
    [CAT, "                                     defaults to HYDRA_ACP_SESSION. With --session, either of"],
    [CAT, "                                     those also prepends a short note to the first prompt"],
    [CAT, "                                     telling the receiving agent the turn came from a peer"],
    [CAT, "                                     rather than its user, and is not authorization;"],
    [CAT, "                                     --no-from-note suppresses it. The daemon bounds"],
    ...
    [SESSION, "                                     --host filters by origin: 'local' (default) shows only sessions created here, 'all' shows everything, or pass a name to show sessions live on the `hydra remote` registered under that name and/or passive imports from a machine of that name."],
    ...
    [SESSION, "  hydra-acp session delete <id>      Delete a session entirely (live or cold). Reverse with `session undelete`."],
