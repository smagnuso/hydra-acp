...
// so the random tail is what users see in lists and pickers. A federated
// id (`name:localId`) has its peer-name prefix stripped first — callers
// already show the remote via its own badge/column, so repeating "name:"
// in every id is redundant — then the same hydra_session_ stripping runs
// on what's left. Anything else (an id not generated by us) passes
// through untouched.
export function stripHydraSessionPrefix(id: string): string {
  const local = parseForeignSessionId(id)?.localId ?? id;
  return local.startsWith(HYDRA_SESSION_PREFIX)
    ? local.slice(HYDRA_SESSION_PREFIX.length)
    : local;
}
